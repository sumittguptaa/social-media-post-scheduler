...
// X's public web app Bearer token (hardcoded in X's web JS, shared by all web users)
const X_WEB_BEARER =
  'AAAAAAAAAAAAAAAAAAAAANRILgAAAAAAnNwIzUejRCOuH5E6I8xnZz4puTs%3D1Zv7ttfk8LF81IUq16cHjhLTvJu4FA33AGWWjCpTnA';

// GraphQL queryId from HAR analysis (CreateTweet endpoint)
const CREATE_TWEET_QUERY_ID = 'lvs5-tN_lLNg_PhdRSURMg';

// Feature flags required by X's GraphQL API (from HAR analysis)
...
export class XCookieProvider extends SocialAbstract implements SocialProvider {
  identifier = 'x-cookie';
  name = 'X (Cookie)';
  ...
  async authenticate(params: {
    ...
    try {
      ...
      const picture = (data.profile_image_url_https || '').replace(
        '_normal',
        ''
      );
  ...
  private buildHeaders(auth_token: string, ct0: string) {
    return {
  ...
  async post(
    ...
    const tweetText = stripHtmlValidation('normal', firstPost.message, true);
